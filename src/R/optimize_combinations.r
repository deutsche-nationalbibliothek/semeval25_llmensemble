...
result_files <- setdiff(
  result_files,
  c("results//predictions.csv", "results//ranked_predictions.csv")
)

names <- str_extract(
  result_files,
  pattern = "^results//(.*)/predictions.arrow$", group = 1
) |>
  str_replace("/", "_")
...
for (i in 1:70) {
  ...
  list_of_predictions <- list_of_predictions  |>
    furrr::future_map(
      .f = ~join_fun(
        x = .x,
        y = list_of_predictions[[neuer_bester]],
