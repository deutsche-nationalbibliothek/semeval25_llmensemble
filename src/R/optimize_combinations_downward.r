...
# you can get the top60_combi.csv or an equivalent ensemble by 
# running combine_results.py

result_files <- paste0(
  "results/", top60$model, "/", top60$prompt, "/predictions.csv"
)

names <- str_extract(
  result_files, pattern = "^results/+(.*)/predictions.csv$", group = 1
)  |>
  str_replace("/", "_")
...
anti_join_fun <- function(
  joined_preds, single_model_preds, n_models, .verbose = FALSE
) {
  ...
  # when score is (almost zero) score.y was the major contributor,
  # so we remove it
  res <- res |>
    filter(abs(score) > eps) |>
    compute_rank()
...
for (i in 1:50) {
  ...
  pred_set_candidates <- list_of_predictions  |>
    furrr::future_map(
      .f = ~anti_join_fun(
        joined_preds = joined_pred_set,
        single_model_preds = .x,
