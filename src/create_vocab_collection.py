...
def parse_vocab(ttl_path: Path, use_altLabels: bool = True, phrase: str = None) -> pd.DataFrame:
    ...
    with ttl_path.open("rb") as f:
        ...
        labels = pd.DataFrame(labels, columns=["label_id", "label_text", "is_prefLabel"])
...
def insert_vocab(
    ...
    if len(this_collection.batch.failed_objects) > 0:
        logging.info(f"Failed to import {len(this_collection.batch.failed_objects)} objects")



def run():
    parser = argparse.ArgumentParser()
    parser.add_argument("--ttl_file", help="Vocab as SKOS ttl file", type=str, required=True)
    parser.add_argument("--collection_name", help="Collection Name in Weaviate", type=str, required=True)
    parser.add_argument("--TEI_port", help="Port on which Hugging Face text embedding service is running", type=str, default='8090')
    parser.add_argument("--phrase", help="Pass a context phrase to globally bias label embeddings", type=str, default=None, required=False)
    parser.add_argument("--overwrite", help="Overwrite existing weaviate collection?", type=str, default=True)
    parser.add_argument("--arrow_out", help="Create a copy of the vocabulary in arrow format", type=str, default=None)
    parser.add_argument("--use_altLabels", help="Create embeddings for SKOS altLabels", type=str, default=True)
