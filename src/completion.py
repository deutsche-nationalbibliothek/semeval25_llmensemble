...
class LLMCompletion:
    ...
    def __init__(
        ...
        self.completion_samplingparams = self.dvc_params["completion"]["vllm"]["samplingparams"]
        ...
        self.vllm_engineargs = EngineArgs(
            ...
            max_model_len = self.vllm_engineargs.get("max_model_len", 15000) if self.hf_model_name in ["meta-llama/Meta-Llama-3.1-70B-Instruct", "mistralai/Mixtral-8x7B-Instruct-v0.1"] else None
            )
        ...
        self.text_types = self.p_general.get("text_types", ["Article", "Book", "Conference", "Report", "Thesis"])
        self.subjects = self.p_general.get("subjects", ["all-subjects", "tib-core-subjects"])
        ...
        try:
            ...
            print("Dataset file not found... expected to read in data in this path: ", dataset_file)
        ...
        self.prompt_frame = self.prompt_builder.build_prompt_by_template(os.path.join(self.prompt_template_dir, self.prompt_template_file))
        self.prompt_length = len(self.tokenizer(self.prompt_frame)["input_ids"])
        self.free_tokens = self.max_total_tokens - self.max_new_tokens - self.prompt_length
        ...
        if self.which_text == "both":
            ...
            raise ValueError("Invalid text type. Choose from 'title', 'abstract', 'both'")
    ...
    def post_process_results(self, response, prompt, doc_id):
        ...
        result = list(set([x.strip() for x in answer.split(", ") if len(x.strip())>1]))
        if self.debug:
            print("Result:",result)
    ...
    def predict(self):
        ...
        batches = [df_documents[i:i + self.batch_size] for i in range(0, df_documents.shape[0], self.batch_size)]
        results = []
        ...
        logging.info("End complete computation at time {}".format(str(datetime.datetime.now())))
        destroy_model_parallel()
        del self.llm
        gc.collect()
        torch.cuda.empty_cache()
        torch.distributed.destroy_process_group()
        logging.info("Done with prediction! Find the results at {}".format(self.completion_file))
...
def execute():
    ...
    parser.add_argument("--hf_model_name", help="Identifier of hugging face model i.e. domain/model", type=str, required=True)
    parser.add_argument("--completion_file", help="Completion Filename/Path", type=str, required=True)
    parser.add_argument("--prompt_specification", help="Path to prompt specification file", type=str, required=True)
    args = parser.parse_args()
    completer = LLMCompletion(args.hf_model_name, args.dataset_file,  args.prompt_specification, args.completion_file)
